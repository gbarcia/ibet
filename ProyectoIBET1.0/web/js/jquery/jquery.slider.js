...
var mc={
    ...
    toggle:function(pulse_set){
        ...
        $('#media-centre-holder').stop().animate({
            height:mc.flag?0:419
        },1000,'swing');
        $('#media-centre').stop().animate({
            top:mc.flag?-419:0
        },1000,'swing',function(){
            try{
        ...
        if($('#pusher').length){
            $('.tml_open').hide();
            $('#pusher div').stop().animate({
                height:mc.flag?0:419
            },1000,'swing',function(){
                $('.tml_open').each(function(){
                    $(this).show().css({
                        top:$(this).offset().top+((mc.flag?1:-1)*120)
                    });
                });
            });
        }
    ...
    pulse:function(s){
        if(!mc.pulse_go)
            ...
            $('#mc-pulse').fadeIn(750,function(){
                mc.pulse(0);
            });
        else
            $('#mc-pulse').fadeOut(750,function(){
                mc.pulse(1);
            });
    },
    checkHash:function(){
    ...
    live:function(){
        ...
        if(mc.flag){
            window.setTimeout(function(){
                mc.play($('p.video:first').attr('rel'));
            },1100);
...
var cmap={
    ...
    close:function(){
        if(!cmap.is_open) return false;
        $('#'+cmap.map_id).fadeOut(cmap.time,function(){
            $('#'+cmap.form_id).fadeIn(cmap.time);
        });
        cmap.is_open=false;
    },
